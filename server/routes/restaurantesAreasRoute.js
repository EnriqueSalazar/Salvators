...
router.post('/findall', jsonParser, (req, res) => {
  ...
  models.restaurantesclientes.findAll(
    {where: payload}
  )
    .then((result) => {
    res.send({
...
router.post('/findone', jsonParser, (req, res) => {
  const payload = req.body;
  models.restaurantesclientes.findOne({
    where: payload
  }).then((result) => {
    res.send({
      result
...
router.post('/create', jsonParser, (req, res) => {
  const payload = req.body;
  models.restaurantesclientes.create(
    payload
  ).then((result) => {
    res.send({
      result: result.id
...
router.post('/update', jsonParser, (req, res) => {
  ...
  const payload = req.body;
  models.restaurantesclientes.update(payload, {
    where: {
      id
    }
  }).then((result) => {
...
router.post('/destroy', jsonParser, (req, res) => {
  const payload = req.body;
  models.restaurantesclientes.destroy({
    where: payload
  }).then((result) => {
    res.send({
      result
